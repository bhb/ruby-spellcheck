# A Ruby port (very similar to original) from http://lojic.com/blog/2008/09/04/how-to-write-a-spelling-corrector-in-ruby/
...
def spelltest(tests)
  n, bad, unknown, = 0, 0, 0
  tests.each do |target, misspellings|
    misspellings.split(' ').each do |misspelling|
      w = correct(misspelling)
      if w!=target
        bad+=1
      end
    end
